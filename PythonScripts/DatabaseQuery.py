...
def getAuthData(PORT, authors, doc, chunk_size = 1000):
    df = pd.DataFrame()
    conn = None
    output = []
    i = 1
    ...
    try:
        ...
        cur = conn.cursor()
        query = "SELECT author_id, doc_content FROM document WHERE author_id IN ("
        flag = False
        for auth in authors:
            if not flag:
...
def getDocData(PORT, doc, chunk_size = 1000):
    df = pd.DataFrame()
    conn = None
    output = []
    i = 1
    try:
        conn = psycopg2.connect(user="stylometry", password="stylometry",
                                database="stylometry_v2", host="localhost", port=PORT)
        cur = conn.cursor()
        query = "SELECT author_id, doc_content FROM document WHERE doc_id = '" + str(doc) + "' ;"
        cur.execute(query)
        print("Execution completed")
        rows = cur.fetchall()
        print("Read completed")
